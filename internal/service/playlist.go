...
func (s *Playlist) GetAll(ctx context.Context, userId int64) ([]dto.Playlist, error) {
    ...
    for i, pl := range playlists {
        count, allowedCount, length := 0, 0, 0
        if pl.Count.Valid { count = int(pl.Count.Int32) }
        if pl.AllowedCount.Valid { allowedCount = int(pl.AllowedCount.Int32) }
        if pl.Time.Valid { length = int(pl.Time.Int32) }
